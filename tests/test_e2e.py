...
def test_end_to_end_pipeline(tmp_path, mocker):
    ...
    # Create an initial seed that triggers the crash
    ...
    bug = triaged_bugs[0]
    assert "buffer-overflow" in bug.crash_type.lower()
    ...
    def mock_apply(p, backup=True):
        ...
        patched_src = source_code.replace("strcpy(buffer, argv[1]);", "if (strlen(argv[1]) < sizeof(buffer)) { strcpy(buffer, argv[1]); } else { }")
    ...
    assert verification_result.verified is True, f"Patch verification failed: {verification_result.failure_reason}"
