...
def test_unknown_target_pipeline(tmp_path):
    ...
    seed_dir.mkdir()
    (seed_dir / "input.txt").write_text("A" * 20)
    ...
    bug = triaged_bugs[0]
    rca = RootCauseReport(root_cause_summary="Buffer overflow in dummy", cwe_class="CWE-121", fix_location="main", vulnerable_functions=["main"])
