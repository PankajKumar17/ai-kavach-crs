...
# The module raises RuntimeError with this message when semgrep is absent.
NO_SEMGREP_MSG = "Semgrep is not installed or not on PATH."


def test_static_scan_finds_vuln():
    """Test that the static scan finds the known unchecked buffer copy in sample_vuln."""
    if not SEMGREP_AVAILABLE:
        with pytest.raises(RuntimeError, match=NO_SEMGREP_MSG):
            run_static_scan(SAMPLE_VULN_DIR, CUSTOM_RULESET)
        pytest.skip("Semgrep not installed; verified the module fails loudly instead.")
