...
def fresh_sample_vuln(tmp_path):
    ...
    for script in ("build_target.ps1", "build_target.sh"):
        ...
        if script_src.exists():
            shutil.copy(script_src, dest.parent / script)
    return dest
...
def test_verify_patch_build_failure(fresh_sample_vuln, dummy_bug, mocker):
    ...
    mocker.patch("ai_kavach.verify.apply_patch", side_effect=mock_apply)
    build_error = __import__('ai_kavach.instrument').instrument.BuildError("Build failed")
    mocker.patch("ai_kavach.verify.build_target", side_effect=build_error)
